...
class AliasControllerTest extends TestCase {
    ...
    public function setUp(): void {
        ...
        $this->aliases[] = $this->createAlias("alias2", "john@example.com", "Second alias", false);
        $this->aliases[] = $this->createAlias("alias3", "john.doe@domain.com", "Third alias", true);
    }
    
    private function createAlias(string $aliasName, string $toMail, string $comment, bool $enabled): Alias {
        $now = new \DateTime('now');
        
        $alias = new Alias();
        $alias->setUserId($this->userId);
